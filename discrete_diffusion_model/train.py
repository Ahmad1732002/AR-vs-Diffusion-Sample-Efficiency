...
# Import EMA - you'll need to add the correct import based on your implementation
# from your_ema_module import EMA
...
parser.add_argument(
    '--data-dir',
    type=str,
    default="/scratch/afraij/shake_speare",
...
# Add embed_dim to model_args
model_args['n_embed'] = args.embed_dim
print(f"Using embedding dimension: {args.embed_dim}")
...
max_iters = 100_000  # total number of training iterations (changed to 100k)
weight_decay = 0.1
beta1 = 0.9
beta2 = 0.98
...
if dataset == "text8":
    ...
    def mmap_data(split: str) -> np.array:
        return np.load(data_dir / f"{split}.bin", mmap_mode="r")
else:
    def mmap_data(split: str) -> np.array:
...
# Calculate dataset size and iterations per epoch
train_data = mmap_data("train")
dataset_size = len(train_data) - seq_len + 1
#iters_per_epoch = dataset_size // (batch_size * ddp_world_size)
data_train = mmap_data("train")
iters_per_epoch = data_train.shape[0] // (batch_size * seq_len)
eval_interval=iters_per_epoch
print(f"Dataset size: {dataset_size:,} tokens")
print(f"Iterations per epoch: {iters_per_epoch:,}")
print(f"Total epochs for {max_iters:,} iterations: {max_iters / iters_per_epoch:.2f}")
...
if master_process:
    ...
    csv_file = out_dir / "log_nanoDD_final1.csv"
    final_csv_file = out_dir / "n_embeds_val_metrics_nanoDD1.csv"
...
while True:
    ...
    # Log at epoch boundaries or regular intervals
    should_log = (current_epoch != last_logged_epoch) or (iter_num == 0) or ((iter_num + 1) % log_interval == 0)
    ...
    if should_log and master_process:
        ...
        if log_to_stdout:
            ...
            print(f"epoch {current_epoch} | iter {iter_num + 1}: loss {lossf:.5f}, {metrics_repr}, time {dt * 1000:.2f}ms")
        if log_to_neptune:
    ...
    if iter_num % eval_interval == 0:
        ...
        val_loss, val_metrics = estimate_val_loss()
        current_epoch_for_eval = (iter_num - 1) // iters_per_epoch  # Use iter_num - 1 for the epoch at evaluation

        if master_process:
            # print val metrics to stdout
            metrics_repr = " | ".join([f"{k} " + f"{v:.5f}" for k, v in val_metrics.items()])
            print(f"val @ epoch {current_epoch_for_eval} (iter {iter_num}): loss {val_loss:.4f}, {metrics_repr}\n")
            ...
            # save checkpoint
            if val_metrics[eval_key] < best_val_loss or always_save_checkpoint:
                best_val_loss = val_metrics[eval_key]
                if iter_num > 0:
                    checkpoint = {
                        ...
                        #torch.save(checkpoint, out_dir / f"ckpt_shakespeare_Aug_12_lr6e4_epoch_num{epoch_cnt}.pt")
    ...
    # termination conditions
    if iter_num >= max_iters or epoch_cnt==51:
        print(f"training complete at {iter_num} iterations.")

        # Final evaluation and logging
        final_val_loss, final_val_metrics = estimate_val_loss()
        final_epoch = (iter_num - 1) // iters_per_epoch

        if master_process:
            print(f"Final validation @ epoch {final_epoch}: loss {final_val_loss:.4f}")

        # Save training logs to CSV

            fieldnames = csv_logs[0].keys()
        ...
        # Save final metrics to separate CSV
            final_metrics = {
                'embed_dim': args.embed_dim,
        ...
        # Add all final training metrics
            for k, v in metrics.items():
                final_metrics[f'final_train_{k}'] = v.item()

        # Add all final validation metrics
            for k, v in final_val_metrics.items():
                final_metrics[f'final_val_{k}'] = v

            with open(final_csv_file, 'a', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=final_metrics.keys())
            # writer.writeheader()
                writer.writerow(final_metrics)
            print(f"Final metrics saved to {final_csv_file}")
